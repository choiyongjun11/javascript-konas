describe('Array에 대해서 학습합니다.', function () {
  it('Array의 기본을 확인합니다.', function () {
    const emptyArr = [];
    expect(Array.isArray(emptyArr)).to.equal(false);
    ...
    expect(multiTypeArr[5][1]).to.be.equal(7);
  });
  ...
  it('Array의 요소(element)를 다루는 방법을 확인합니다.', function () {
    ...
    expect(poppedValue).to.equal(expected);
    expect(arr).to.deep.equal([1, 2]);
  });

  it('Array 메소드 slice를 확인합니다.', function () { //slice는 깊은 복사
    const arr = ['peanut', 'butter', 'and', 'jelly'];

    expect(arr.slice(1)).to.deep.equal(['butter','and','jelly']); // 1~ 끝까지
    expect(arr.slice(0, 1)).to.deep.equal(['peanut']); //0~1까지 peanut
    ...
    expect(arr.slice(0)).to.deep.equal(['peanut', 'butter', 'and', 'jelly']); //깊은 복사
  });
